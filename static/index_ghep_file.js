...
var Data = new Array(10000).fill(null).map(function (i) {
  return i = new Array(500).fill(null);
});
var text_formular = new Array(10000).fill(null).map(function (i) {
  return i = new Array(500).fill(null);
});
var index_formular = new Array(10000).fill(null).map(function (i) {
  return i = new Array(500).fill(null);
});
var formular = [];

function Table_hieu_2(props) {
  var myname = 'Table_hieu_2-';
  ...
  var limit_col_view;
  useEffect(function () {
    document.body.style.zoom = "100%"; // ngăn cản zoom bằng ctr
    ...
    _onMouseEnter_not_event(0, 0, 0, 0); // canvas làm mất lắng nghe sự kiện wheel ở table_excel khi vị trí mouse ở canvas
    // thêm sự kiện này để khi cuộn canvas table_excel di chuyển


    canvas_.current.addEventListener("wheel", function (evt) {
  ...
  function dia_chi_o_click(dia_chi_o_click_array_2d_row, dia_chi_o_click_array_2d_col, dia_chi_o_click_in_view_row, dia_chi_o_click_in_view_col) {
    ...
    vi_tri_click_in_Data[1] = dia_chi_o_click_array_2d_col; // set địa chỉ ô click trong khung nhìn hiển thị
  ...
  function run_function_when_input_focus(input_, i, j, i_array_2d, j_array_2d) {
    var input_formula = thanh_dia_chi_0.current; // khi di chuyển chuột trong thẻ input xác định vị trí của con chuột tới vị trí nào trong text input
    ...
    input_.onkeydown = function (event) {
      ...
      console.log("---------------------_input_.onkeydown"); // khi ấn phím khác enter thì tiến hành ghi dữ liệu vào thẻ input
      ...
      if (event.keyCode !== 13) {
        ...
        console.log('vi_tri_con_tro_khi_di_chuyen_trong_double_click_input                     ' + vi_tri_con_tro_khi_di_chuyen_trong_double_click_input); //**************** */ trong javscript thuần ghi giá trị từ bàn phím vào thẻ input sẽ diến ra sau việc lấy giá trị từ thẻ input vào biến.
      ...
      if (event.keyCode === 13) {
        ...
        console.log(onKeyDown_1_element); //  key_enter chạy xong cho onKeyDown_1_element = false
        ...
        setTimeout(function () {
          console.log('setTimeout---------------- để onKeyDown_1_element = true .   cuối cùng mới chuyển sang flase'); // khi ấn enter thì công thức chưa hoàn thành cũng tính

          tinh_toan(i, j, "xoa_ky_tu_cong_thuc_thua"); // set địa chỉ ô click  sau hành động trên
  ...
  function change_input_scroll(row_vi_tri_remove, col_vi_tri_remove, row_vi_tri_add, col_vi_tri_add) {
    var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
    var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML); //  xoá tô màu  vị trí trước đó: nếu vị trí trước đó  không nằm trong khung nhìn thì xoá focus đang hiện diện ngược lại xoá tô màu
    ...
    if (row_vi_tri_add < 0 || row_vi_tri_add > limit_view - 1 || col_vi_tri_add < 0 || col_vi_tri_add > limit_col_view - 1) {
      ...
      vi_tri_o_truoc[1] = col_vi_tri_add; // set địa chỉ ô click  sau hành động trên
      ...
      console.log('xuất hiện lại thẻ input và tô màu:  ' + row_vi_tri_add + '-' + col_vi_tri_add); // phải focus thẻ div dể thẻ đó nhận sự kiện từ bàn phím được
      ...
      input_.value = text_formular[row_vi_tri_add + i_array_2d][col_vi_tri_add + j_array_2d]; // thay đổi độ rộng của input phù hợp với ký tự nhập vào
      ...
      setTimeout(function () {
        ...
        run_function_when_input_focus(input_, row_vi_tri_add, col_vi_tri_add, i_array_2d, j_array_2d); // set địa chỉ ô click  sau hành động trên
  ...
  function key_enter(row_vi_tri_remove, col_vi_tri_remove, row_vi_tri_add, col_vi_tri_add) {
    ...
    onKeyDown_1_element = false;
    thanh_dia_chi_0_on_keydown = false;
    turn_off_onMouseEnter = false; //1.  xoá tô màu vị trí trước
    ...
    if (row_vi_tri_add < 0 || row_vi_tri_add > limit_view - 1 || col_vi_tri_add < 0 || col_vi_tri_add > limit_col_view - 1) {
      ...
      console.log('tô màu:  ' + row_vi_tri_add + '-' + col_vi_tri_add); // phải focus thẻ div dể thẻ đó nhận sự kiện từ bàn phím được
      // muốn focus thẻ div phải đặt tabindex cho thẻ đó
      // ** ảnh hưởng của input và button khi click sẽ làm mất đường viền đen focus của thẻ div ; chính là thuộc tính Outline  của css
  ...
  function tinh_toan(i, j, z) {
    ...
    var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
    var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML);
    var text = text_formular[i + i_array_2d][j + j_array_2d]; // chuyển text null bằng '' để slice không bị lỗi.
    ...
    var text_den_vi_tri_con_tro = text.slice(0, vi_tri_con_tro_khi_di_chuyen_trong_double_click_input); // z chỗ này chạy khi tính toán bằng cách bấm enter    
    // kết thúc công thức toàn bộ text co nhiều dấu chẳng hạn +9+6+++ thì xoá hết dấu + ở cuối đi

    if (z === "xoa_ky_tu_cong_thuc_thua") {
      console.log('text_den_vi_tri_con_tro           ' + text_den_vi_tri_con_tro); // khi bấm enter z = xoa_ky_tu_cong_thuc_thua ta xoá 1 ký tự công thức  cuối chỗ vị trí con trỏ để công thức không hoàn thành cũng tính
    ...
    if (text.slice(0, 1) === "=" || text.slice(0, 1) === "+") {
      ...
      try {
        ...
        kiem_tra_loi = true;
        console.log(kiem_tra_loi); // kiểm tra xem trong mảng formular có tồn tại vị trí index được lưu chưa. Nếu có thay đổi công thức ở vị trí đó.
        ...
        if (index_formular[i + i_array_2d][j + j_array_2d] !== null) {
          ...
          formular[index_](); // tính toán lại tất cả các công thức đã viết

          var _len = formular.length;
          ...
          for (var _index = 0; _index <= limit_view; _index++) {
            for (var index_j = 0; index_j <= limit_col_view; index_j++) {
              a.current.children[_index + 1].children[index_j + 1].innerHTML = Data[_index + i_array_2d][index_j + j_array_2d];
            }
          }
          ...
          index_formular[i + i_array_2d][j + j_array_2d] = formular.length - 1; // tính 1 công thức cuối cùng

          formular[formular.length - 1](); // tính toán lại tất cả các công thức đã viết

          var _len2 = formular.length;

          for (var _index2 = 0; _index2 < _len2; _index2++) {
            if (formular[_index2] !== undefined) {
              formular[_index2]();
            }
      ...
      if (kiem_tra_loi === undefined) {
        ...
        if (index_formular[i + i_array_2d][j + j_array_2d] !== null) {
          index_ = index_formular[i + i_array_2d][j + j_array_2d];
          console.log("khac nhau"); // kiểm tra tham chiếu lặp

          var array_loi_tham_chieu = [];
          var vi_tri_loi_tham_chieu;
          text_formular.map(function (item, index) {
            item.map(function (j_item, index_j) {
              if (j_item !== null) {
          ...
          if (array_loi_tham_chieu.some(function (item, index) {
          ...
          var _len3 = formular.length;

          for (var _index3 = 0; _index3 < _len3; _index3++) {
            console.log(formular[_index3]);

            if (formular[_index3] !== undefined) {
              formular[_index3]();
            }
          ...
          for (var _index4 = 0; _index4 <= limit_view; _index4++) {
            for (var _index_j = 0; _index_j <= limit_col_view; _index_j++) {
              a.current.children[_index4 + 1].children[_index_j + 1].innerHTML = Data[_index4 + i_array_2d][_index_j + j_array_2d];
            }
          }
          ...
          console.log("push 1 cong thuc vao cuoi ----------------"); // kiểm tra tham chiếu lặp

          var array_loi_tham_chieu = [];

          var _vi_tri_loi_tham_chieu;
          ...
          if (array_loi_tham_chieu.some(function (item, index) {
            _vi_tri_loi_tham_chieu = item;
            return text.indexOf("Data[" + item[0] + "][" + item[1] + "]") !== -1;
          }) === true) {
            formular.push(eval("(function(){return " + "Data[" + (i + i_array_2d) + "][" + (j + j_array_2d) + "]" + " =" + "'#REF!'" + ";})"));
            Data[_vi_tri_loi_tham_chieu[0]][_vi_tri_loi_tham_chieu[1]] = "#REF!";
            alert("Lỗi tham chiếu tại " + "Data[" + _vi_tri_loi_tham_chieu[0] + "][" + _vi_tri_loi_tham_chieu[1] + "]");
          } else {
          ...
          var _len4 = formular.length;

          for (var _index5 = 0; _index5 < _len4; _index5++) {
            if (formular[_index5] !== undefined) {
              formular[_index5]();
            }
          ...
          if (i <= limit_view - 1 & i >= 0 && j <= limit_col_view - 1 & j >= 0) {
            ...
            if (_vi_tri_loi_tham_chieu !== undefined) {
              a.current.children[_vi_tri_loi_tham_chieu[0] + 1 - i_array_2d].children[_vi_tri_loi_tham_chieu[1] + 1 - j_array_2d].innerHTML = Data[_vi_tri_loi_tham_chieu[0]][_vi_tri_loi_tham_chieu[1]];
            }
          }
    ...
    if (text.slice(0, 1) != "=" && text.slice(0, 1) != "+") {
      ...
      setTimeout(function () {
        thanh_dia_chi_0.current.value = "";
      }, 0); // nếu người dùng nhập số  vào input thì hàm + text  sẽ chuyển ký tự khác số thành số. Vd: có nhiều phím cách "  59" sẽ chuyển thành 59, "  .2" sẽ chuyển thành 2 

      if (isNaN(Number(text)) == false) {
        Data[i + i_array_2d][j + j_array_2d] = +text;
        console.log("Data[" + (i + i_array_2d) + "][" + (j + j_array_2d) + "]       " + Data[i + i_array_2d][j + j_array_2d]); // nếu người dùng không nhập gì thì isNaN(Number(text))  sẽ trả về false khi đó ta gán Data[i+i_array_2d][j] = '' ;
        ...
        if (index_formular[i + i_array_2d][j + j_array_2d] !== null) {
          index_ = index_formular[i + i_array_2d][j + j_array_2d];
          console.log("khac nhau"); // xoá công thức ở vị trí đó  nhưng vẫn để lại trong index_formular vị trí index_

          delete formular[index_];
        } // tính toán lại tất cả các công thức đã viết


        var _len5 = formular.length;

        for (var _index6 = 0; _index6 < _len5; _index6++) {
          if (formular[_index6] !== undefined) {
            formular[_index6]();
          }
        ...
        for (var _index7 = 0; _index7 <= limit_view; _index7++) {
          for (var _index_j2 = 0; _index_j2 <= limit_col_view; _index_j2++) {
            a.current.children[_index7 + 1].children[_index_j2 + 1].innerHTML = Data[_index7 + i_array_2d][_index_j2 + j_array_2d];
          }
        }
      ...
      if (isNaN(Number(text)) == true) {
        Data[i + i_array_2d][j + j_array_2d] = text; // kiểm tra xem trong mảng formular có tồn tại vị trí index được lưu chưa. Nếu có xoá công thức ở vị trí đó  
        ...
        var _len6 = formular.length;

        for (var _index8 = 0; _index8 < _len6; _index8++) {
          if (formular[_index8] !== undefined) {
            formular[_index8]();
          }
        ...
        for (var _index9 = 0; _index9 <= limit_view; _index9++) {
          for (var _index_j3 = 0; _index_j3 <= limit_col_view; _index_j3++) {
            a.current.children[_index9 + 1].children[_index_j3 + 1].innerHTML = Data[_index9 + i_array_2d][_index_j3 + j_array_2d];
          }
        }
  ...
  function _onKeyDown(event, i, j) {
    console.log('_onKeyDown------------------------------'); //  onKeyDown_1_element === false tức chưa xuất hiện thẻ input
    ...
    if (onKeyDown_1_element === false) {
      var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
      var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML); // 1. element được kích hoạt và element không ở trạng thái tính toán mà bấm phím enter sẽ xuống dòng 
      ...
      if (event.key == "Enter") {
        ...
        console.log("khi bấm phím enter mà không xuất hiện thẻ input thì không phải tính toán"); // set địa chỉ ô click  sau hành động trên
      ...
      if (event.key != "Enter") {
        console.log("_onKeyDown--nhập dữ liệu"); // khi ấn phím khác enter thì viết công thức hoặc dữ liệu vào ô đó (thiết lập ô đó ở trạng thái tính toán)
        ...
        Object.assign(input_.style, css.input_focus); // lúc này input nhận giá trị từ bàn phím hiện lên thẻ input 
        ...
        vi_tri_o_truoc[0] = i;
        vi_tri_o_truoc[1] = j; // trước gán sự kiện keydown cho input thì ta phải tắt lắng nghe sự kiện onkedown cho 1 element table cha của input 
  ...
  var turn_off_onMouseEnter = false; // vẽ khung miền lựa chọn và nếu thoả mãn điều kiện thì vẽ biểu tượng fill

  function _onMouseEnter_not_event(x, y, i, j, thay_doi_mien_select_array_2d) {
    console.log('vẽ canvas');
    console.log(mien_select_array_2d); // xoá biểu tượng fill đã xuất hiện khi move mouse
    ...
    css.canvas_.left = x_r0c0 - ref_0.current.getBoundingClientRect().x + 'px'; // css.canvas_.borderRight = "2px solid #00A170";

    if (x_r0c0 - ref_0.current.getBoundingClientRect().x + (x_r0c1 - x_r0c0 - 4) >= table_excel.current.clientWidth) {
      css.canvas_.width = table_excel.current.clientWidth - (x_r0c0 - table_excel.current.getBoundingClientRect().x) + 'px'; // css.canvas_.borderRight = "none";
    }

    if (y_r0c0 - table_excel.current.getBoundingClientRect().y + (y_r1c0 - y_r0c0 - 4) >= table_excel.current.clientHeight) {
      css.canvas_.height = table_excel.current.clientHeight - (y_r0c0 - table_excel.current.getBoundingClientRect().y) + 'px'; // css.canvas_.borderRight = "none";
    } // vẽ lại  biểu tượng fill 


    var ty_le_canvas_width = table_excel_width / (x_r0c1 - x_r0c0 - 4);
    var ty_le_canvas_height = table_excel_height / (y_r1c0 - y_r0c0 - 4);
  ...
  function _onMouseEnter(event, x, y, i, j) {
    ...
    if (event.buttons == 1) {
      _onMouseEnter_not_event(x, y, i, j); // khi nhấn chuột trái và di chuyển trong box canvas_ thì ẩn canvas_ đi
      ...
      canvas_.current.onmousemove = function (e) {
        ...
        var y = e.clientY;
        var x = e.clientX;
        var a_1 = vi_tri_o_truoc[0];
        var b_1 = vi_tri_o_truoc[1];
        ...
        if (a_1 <= mien_select[2] && b_1 > mien_select[3]) {
          ...
          if (e.buttons == 1 && x >= X && y <= Y) {
            ...
            console.log(mien_select[2], mien_select[3] + 1); // tới ô cách ô cuối theo đường chéo 1 ô
            ...
            console.log('ve lai trong canvas_--- duoi trai'); // tới ô cách ô cuối theo đường chéo 1 ô
        ...
        if (a_1 > mien_select[2] && b_1 <= mien_select[3]) {
          ...
          if (e.buttons == 1 && x <= X && y >= Y) {
            console.log('ve lai trong canvas_--- tren phai'); // tới ô cách ô cuối theo đường chéo 1 ô
        ...
        if (a_1 > mien_select[2] && b_1 > mien_select[3]) {
          ...
          if (e.buttons == 1 && x >= X && y >= Y) {
            console.log('ve lai trong canvas_--- tren trai'); // tới ô cách ô cuối theo đường chéo 1 ô
  ...
  var xuat_hien_the_input = false;
  var vi_tri_con_tro_khi_di_chuyen_trong_double_click_input;
  ...
  function _onMouseDown(_this, i, j, event) {
    ...
    var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
    var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML); // đầu tiên nó chạy hàm 1 click khởi tạo kiem_tra = 1 do đó khi click thêm trong vòng 300 ms giây thì kiem_tra vẫn = 1 => nó chạy hàm 2 click
    ...
    if (_this.kiem_tra == 1) {
      ...
      vi_tri_o_truoc[1] = j;
      _onKeyDown2 = true;
      var input_ = _this.children[0];
      Object.assign(input_.style, css.input_focus);
      input_.value = text_formular[i + i_array_2d][j + j_array_2d]; // thay đổi độ rộng của input phù hợp với ký tự nhập vào
      ...
      console.log('vi_tri_con_tro_khi_di_chuyen_trong_double_click_input                     ' + vi_tri_con_tro_khi_di_chuyen_trong_double_click_input); //   setTimeout ở đây vì  khi double click thì sẽ chạy cả 1 click trước trong đó cũng có chỗ setTimeout => setTimeout để đoạn code này chạy cuối cùng                                                          
      //        setTimeout(() => {
    ...
    if ((_this.kiem_tra == 0 || _this.kiem_tra == undefined) && _typeof(_this.children[0]) != "object") {
      // huỷ bỏ miền chọn cũ
      Object.assign(canvas_.current.style, {
        display: "none"
      });
      ...
      vi_tri_khung_nhin_truoc_scroll[1] = j_array_2d; // thiết lập để  element div này không lắng nghe sự kiện onKeyDown  (khi dùng chuột ta sẽ khoá nhận bàn phím)

      _onKeyDown2 = false;
      console.log("_onClick---"); //1.----------------------------------------------------------------
      ...
      if (cong_thuc_chua_hoan_thanh === "") {
        ...
        if (vi_tri_o_truoc[0] < 0 || vi_tri_o_truoc[0] > limit_view - 1 || vi_tri_o_truoc[1] < 0 || vi_tri_o_truoc[1] > limit_col_view - 1) {} else {
          a.current.children[vi_tri_o_truoc[0] + 1].children[vi_tri_o_truoc[1] + 1].innerHTML = Data[vi_tri_o_truoc[0] + i_array_2d][vi_tri_o_truoc[1] + j_array_2d];
        }

        console.log("_onClick---công thức hoàn thành = '', tô màu"); // set địa chỉ ô click  sau hành động trên

        dia_chi_o_click(i + i_array_2d, j + j_array_2d, i, j); // tô màu vào vị trí i,j
      ...
      if (cong_thuc_chua_hoan_thanh != "") {
        console.log("_onClick---viet tiếp công thức vào thẻ input"); // viet tiếp công thức vào thẻ input nên phải  để  onKeyDown_1_element = true; để không lắng nghe sụ kiện keydown ở element cha của input

        onKeyDown_1_element = true;
        console.log(cong_thuc_chua_hoan_thanh); // nếu công thức đang viết dở thì khi onclick sẽ viết tiếp công thức vào ô trước đó.
        ...
        var c = parseInt(cong_thuc_chua_hoan_thanh[1]); // gán công thức cũ vào text

        var text = cong_thuc_chua_hoan_thanh[2];
        var len_text = text.length; // khi lần thứ 2 nhấn vào ô khác mà không di chuyển con trỏ thì vi_tri_con_tro_khi_di_chuyen_trong_double_click_input vẫn như cũ
        ...
        console.log(cong_thuc_them_vao[0]); // click vào ô khác lần 2,3 thì input_truoc_do.cong_thuc_them_vao[0] và  input_truoc_do.vi_tri_cong_thuc_them_vao[0]  đã được set value
        ...
        var text_update = text.slice(0, vi_tri_con_tro_khi_di_chuyen_trong_double_click_input) + "(Data[" + (i + i_array_2d) + "][" + (j + j_array_2d) + "])" + text.slice(vi_tri_con_tro_khi_di_chuyen_trong_double_click_input, len_text);
        ...
        console.log(cong_thuc_them_vao[0]); // khi scroll nếu input trước đó nằm trong khung nhìn thì viết tiếp công thức vào input đó
        ...
        if (r <= limit_view - 1 & r >= 0 && c <= limit_col_view - 1 && c >= 0) {
          ...
          cong_thuc_chua_hoan_thanh[2] = text_update; // thay đổi độ rộng của input phù hợp với ký tự nhập vào
          ...
          var vi_tri_focus = vi_tri_con_tro_khi_di_chuyen_trong_double_click_input + cong_thuc_them_vao[0].length;
          console.log('vi_tri_focus      ' + vi_tri_focus);
          console.log('vi_tri_con_tro_khi_di_chuyen_trong_double_click_input      ' + vi_tri_con_tro_khi_di_chuyen_trong_double_click_input);
          setTimeout(function () {
            input_truoc_do.focus({
              preventScroll: true
            });
            input_truoc_do.setSelectionRange(vi_tri_focus, vi_tri_focus);
          }, 0); // hiện công thức đó lên thanh formular để người dùng viết tiếp.
          ...
          text_formular[r + i_array_2d][c + j_array_2d] = text_update;
          cong_thuc_chua_hoan_thanh[2] = text_formular[r + i_array_2d][c + j_array_2d]; // hiện công thức đó lên thanh formular để người dùng viết tiếp.

          var input_formula = thanh_dia_chi_0.current;
          input_formula.value = text_formular[r + i_array_2d][c + j_array_2d];
          input_formula.vi_tri = [r + i_array_2d, c + j_array_2d]; // thay đổi độ rộng của input phù hợp với ký tự nhập vào
        ...
        onclick_tinh_toan = true; // để tắt input lắng nghe sự kiện bàn phím trên all element lúc render
      ...
      _this.onkeydown = function (event) {
        ...
        if (onKeyDown_1_element === false) {
          console.log("_onMouseDown_onKeyDown"); // enter-----xuất hiện thẻ input khi double click mà không nhập gì chỉ ấn enter cũng không cần tính toán
          ...
          if (event.keyCode === 13) {
            ...
            a.current.children[i + 1].children[j + 1].innerHTML = Data[i + i_array_2d][j + j_array_2d]; // set địa chỉ ô click  sau hành động trên
          ...
          if (event.keyCode != 13) {
            console.log("_onMouseDown_onKeyDown_keyCodekhac enter"); // khi ấn phím khác enter thì viết công thức hoặc dữ liệu vào ô đó (thiết lập ô đó ở trạng thái tính toán)

            a.current.children[i + 1].children[j + 1].innerHTML = " <input   type='text'  />"; // biến này dùng để xác định ảnh hưởng tới scoll

            xuat_hien_the_input = true;
            var input_ = a.current.children[i + 1].children[j + 1].children[0];
            Object.assign(input_.style, css.input_focus);
            var input_formula = thanh_dia_chi_0.current; // gán giá trị công thức cũ vào thẻ input. Sự kiện này sẽ diễn ra trước sự kiện thêm 1 ký tự khi ấn bàn phím

            input_.value = text_formular[i + i_array_2d][j + j_array_2d]; // thay đổi độ rộng của input phù hợp với ký tự nhập vào trước khi ấn phím
            ...
            console.log(cong_thuc_them_vao[0]); // nếu công thức chưa hoàn thành khác ""  thì di chuyển con trỏ tới vị trí sau khi viết thêm công thức

            if (cong_thuc_chua_hoan_thanh != "") {
              vi_tri_con_tro_khi_di_chuyen_trong_double_click_input = vi_tri_con_tro_khi_di_chuyen_trong_double_click_input + cong_thuc_them_vao[0].length;
            ...
            onclick_tinh_toan = true;
            vi_tri_o_truoc[0] = i;
            vi_tri_o_truoc[1] = j; // tắt lắng nghe sự kiện onKeyDown cho div element
  ...
  function copy(event) {
    mien_copy = [].concat(mien_select_array_2d);
    var text_copy = "";

    for (var index = mien_copy[0]; index < mien_copy[2]; index++) {
      var _text_copy_row = ""; // đầu tiên nó truyền index = mien_copy[0] xuống for sau đó nó lặp qua các cột ghép thành text dòng

      for (var index_j = mien_copy[1]; index_j < mien_copy[3]; index_j++) {
        _text_copy_row = _text_copy_row + Data[index][index_j] + "\t";
      } // cuối cùng của text_copy_row không có "\t"


      _text_copy_row = _text_copy_row + Data[index][mien_copy[3]]; // ghép tất cả các dòng lại tạo thành text table copy tới excel được

      text_copy = text_copy + _text_copy_row + "\r";
    } // cuối cùng của text_copy tại mien_copy[2] không có "\r"


    var text_copy_row = "";

    for (var _index_j4 = mien_copy[1]; _index_j4 < mien_copy[3]; _index_j4++) {
      text_copy_row = text_copy_row + Data[mien_copy[2]][_index_j4] + "\t";
    } // cuối cùng của text_copy_row không có "\t"
  ...
  function paste(event) {
    ...
    navigator.clipboard.readText().then(function (clipText) {
      ...
      var array_paste;
      array_paste = clipText.split("\r"); // chú ý xem lại sử sụng let trong vòng lặp for khác var

      for (var index = 0, len = array_paste.length; index < len; index++) {
        array_paste[index] = array_paste[index].split("\t");

        for (var index_col = 0, len_col = array_paste[0].length; index_col < len_col; index_col++) {
      ...
      for (var _index10 = 0; _index10 <= limit_view; _index10++) {
        for (var index_j = 0; index_j <= limit_col_view; index_j++) {
          a.current.children[_index10 + 1].children[index_j + 1].innerHTML = Data[_index10 + i_array_2d][index_j + j_array_2d];
        }
      }
    });
  } //////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  var trang_thai_fill = false;
  var kieu_fill;

  function fill(event) {
    var text = text_formular[vi_tri_click_in_Data[0]][vi_tri_click_in_Data[1]];
    console.log(text);
    var text_replace;
    var kq;
    var text_chi_chua_value_hoac_cong_thuc = false;
    var text_cong_thuc_cell_next;
    ...
    if (text === null) {
      ...
      if (text.indexOf("Data") === -1) {
        ...
        text_replace = text.replaceAll('Data', '|_|Data'); // tạo mảng lưu trữ text công thức viết

        kq = text_replace.split('|_|');
      }
    }

    var keo_doc = mien_select_array_2d[2] - mien_select_array_2d[0];
    var keo_ngang = mien_select_array_2d[3] - mien_select_array_2d[1];
    var data_array_2d = [];

    var _loop = function _loop(index_ngang) {
      var data_array_col = [];

      var _loop2 = function _loop2(index_doc) {
        // fill xong thì lưu vào mảng text_cong_thuc_cell_next
        if (text_chi_chua_value_hoac_cong_thuc === true) {
          data_array_col.push(text);
        } else {
          text_cong_thuc_cell_next = kq.map(function (i) {
            ...
            var i_row = i.match(/.*Data\$?\[[0-9]+\]/i); //  x(?!y)	Chỉ khớp x nếu ngay sau x không phải là y

            var i_col = i.match(/\$?\[[0-9]+\](?!\[).*/i); //  i_col[0] =   i_col[0].slice(1) ;
            ...
            if (i_row != null) {
              ...
              var co_dinh_row = /\$/i.test(i_row[0]); // kiểm tra trước cột có kí tự $ không

              var co_dinh_col = /\$/i.test(i_col[0]);

              if (co_dinh_col === false) {
                // đâ thoả mãn điều kiện tiến hành fill    
                var text_moi = i_col[0].match(/[0-9]+/i)[0] * 1 + index_doc;
                var i_col_replace = i_col[0].replace(/[0-9]+/i, text_moi);
                i_col = i_col_replace;
              }
              ...
              if (co_dinh_row === false) {
                // đâ thoả mãn điều kiện tiến hành fill  
                var _text_moi = i_row[0].match(/[0-9]+/i)[0] * 1 + index_ngang;

                var i_replace = i_row[0].replace(/[0-9]+/i, _text_moi) + i_col;
                return i_replace;
              }
      ...
      for (var index_doc = 0; index_doc <= keo_ngang; index_doc++) {
        _loop2(index_doc);
    ...
    for (var index_ngang = 0; index_ngang <= keo_doc; index_ngang++) {
      _loop(index_ngang);
    }

    console.log(data_array_2d);
    var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
    var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML); // lặp trong mảng data_array_2d lấy công thức push vào formular

    for (var index = 0, len = keo_doc; index <= len; index++) {
      for (var index_col = 0, len_col = keo_ngang; index_col <= len_col; index_col++) {
        if (index === 0 && index_col === 0) {} else {
          var index_ = index_formular[vi_tri_o_truoc[0] + i_array_2d + index][vi_tri_o_truoc[1] + j_array_2d + index_col];
          ...
          if (index_ !== null) {
            ...
            if (text.slice(0, 1) !== "=" && text.slice(0, 1) !== "+") {
              ...
              text_formular[vi_tri_o_truoc[0] + i_array_2d + index][vi_tri_o_truoc[1] + j_array_2d + index_col] = data_array_2d[index][index_col];
              delete formular[index_]; // nếu fill ô chứa công thức mà trong công thức chứa   Data thì push vào formular
            } else {
              formular[index_] = eval("(function(){return " + "Data[" + (vi_tri_o_truoc[0] + i_array_2d + index) + "][" + (vi_tri_o_truoc[1] + j_array_2d + index_col) + "]" + " = " + data_array_2d[index][index_col] + ";})"); // tính toán lại công thức mới ghi
            ...
            if (text.slice(0, 1) !== "=" && text.slice(0, 1) !== "+") {
              ...
              formular.push(eval("(function(){return " + "Data[" + (vi_tri_o_truoc[0] + i_array_2d + index) + "][" + (vi_tri_o_truoc[1] + j_array_2d + index_col) + "]" + " = " + data_array_2d[index][index_col] + ";})")); // tính toán lại công thức mới ghi
    ...
    var _len = formular.length; // tính toán lại tất cả các công thức đã viết

    for (var _index11 = 0; _index11 < _len; _index11++) {
      if (formular[_index11] !== undefined) {
        formular[_index11]();
      }
    ...
    for (var _index12 = 0; _index12 <= limit_view; _index12++) {
      for (var index_j = 0; index_j <= limit_col_view; index_j++) {
        a.current.children[_index12 + 1].children[index_j + 1].innerHTML = Data[_index12 + i_array_2d][index_j + j_array_2d];
      }
    }
  ...
  function fill_2(event) {
    ...
    var keo = keo_doc + keo_ngang;
    console.log(keo_doc, keo_ngang, keo);
    var data_array_2d = [];
    var x = 0;
    var y = 0;

    for (var index = 0; index <= keo; index++) {
      if (kiem_tra) {
        x = index;
      } else {
        y = index;
      ...
      ;
      var text = text_formular[vi_tri_click_in_Data[0] + x][vi_tri_click_in_Data[1] + y];
      console.log(text);
      var text_replace = void 0;
      var kq = void 0;
      var text_chi_chua_value_hoac_cong_thuc = false;
      var text_cong_thuc_cell_next = void 0;
      ...
      var _loop3 = function _loop3(index_ngang) {
        var data_array_col = [];

        var _loop4 = function _loop4(index_doc) {
          // fill xong thì lưu vào mảng text_cong_thuc_cell_next
          if (text_chi_chua_value_hoac_cong_thuc === true) {
            data_array_col.push(text);
          } else {
            text_cong_thuc_cell_next = kq.map(function (i) {
              ...
              if (i_row != null) {
                ...
                if (co_dinh_row === false) {
                  // đâ thoả mãn điều kiện tiến hành fill  
                  var _text_moi2 = i_row[0].match(/[0-9]+/i)[0] * 1 + index_ngang;

                  var i_replace = i_row[0].replace(/[0-9]+/i, _text_moi2) + i_col;
                  return i_replace;
                }
  ...
  var table_excel_height = window.innerHeight - 87.742 - 60;
  var table_excel_width = window.innerWidth - 40;
  var limit = 100;
  var limit_col = 50;
  var Data_show = Data.slice(0, limit);
  var Data_show_0 = Data_show.map(function (item, index) {
    return item.slice(0, limit_col);
  }); // ở zoom 100 % 1 click scroll ở chrome di chuyển 40 pixcel 

  var zoom = window.devicePixelRatio;
  var click_scroll_dichuyen = 40 / zoom;
  var data_lenght = 10000 * click_scroll_dichuyen;
  var data_col_lenght = 500 * click_scroll_dichuyen;
  ...
  function _onScroll(event) {
    ...
    var vi_tri_cat_truoc_do = a.current.children[0 + 1].children[0].innerHTML; // là vị trí cắt trước đó

    var i_array_2d = parseInt(a.current.children[0 + 1].children[0].innerHTML);
    var j_array_2d = parseInt(a.current.children[0].children[0 + 1].innerHTML); // scrollHeight chiều cao của cả thanh scroll
    ...
    if (di_chuyen <= data_lenght - click_scroll_dichuyen * 100) {} else {
    ...
    if (di_chuyen_col <= data_col_lenght - click_scroll_dichuyen * 10) {} else {
    ...
    var vi_tri_cat_truoc_do_col = a.current.children[0].children[0 + 1].innerHTML; // là vị trí cắt trước đó
    ...
    limit_col_view = sum_col - 1; // cập nhật lại dữ liệu khi scroll -- bước1

    for (var index = 0; index <= limit_view; index++) {
    ...
    if (xuat_hien_the_input) {
      ...
      console.log('key_enter:   ' + vi_tri_o_truoc[0] + '__' + vi_tri_o_truoc[1] + '__' + (vi_tri_o_truoc[0] - (vi_tri_cat - vi_tri_cat_truoc_do)) + '__' + (vi_tri_o_truoc[1] - (vi_tri_cat_col - vi_tri_cat_truoc_do_col))); // set địa chỉ ô click  sau hành động trên
    ...
    console.log("---------/////////////////////////////////////////////////////////////--------------------------");
    console.log(vi_tri_o_truoc); // bước 4 
    // vịtrí ô viết công thức không nằm trong khung nhìn thì focus tại thanh công thức
    ...
    if (position_mouse_brower === 'ouside_brower') {
      ...
      var x = vi_tri_o_truoc[0];
      var y = vi_tri_o_truoc[1];
      var i = mien_select[2];
      var j = mien_select[3];
      console.log(x, y, i, j);
    ...
    if (position_mouse_brower === undefined) {
      ...
      var _x = vi_tri_o_truoc[0];
      var _y = vi_tri_o_truoc[1];
      var _i = mien_select[2];
      var _j = mien_select[3];
      console.log(_x, _y, _i, _j);
      ...
      if (_x > limit_view - 1 && _i > limit_view - 1) {
        return Object.assign(canvas_.current.style, {
          display: "none"
        });
      }

      if (_y > limit_col_view && _j > limit_col_view) {
        return Object.assign(canvas_.current.style, {
          display: "none"
        });
      }
      ...
      if (_j > limit_col_view) {
        _j = limit_col_view;
      }

      _onMouseEnter_not_event(_x, _y, _i, _j, false);
    }
    ...
    if (position_mouse_brower === 'on_thanh_dia_chi_onMouseDown') {
      ...
      mien_select[2] = vi_tri_o_truoc[0] + mien_select_array_2d[2] - mien_select_array_2d[0];
      mien_select[3] = vi_tri_o_truoc[1] + mien_select_array_2d[3] - mien_select_array_2d[1];
      var _x2 = vi_tri_o_truoc[0];
      var _y2 = vi_tri_o_truoc[1];
      var _i2 = mien_select[2];
      var _j2 = mien_select[3];
      ...
      console.log(_x2, _y2, _i2, _j2);

      _onMouseEnter_not_event(_x2, _y2, _i2, _j2, false);
    }
  } ////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  function thanh_dia_chi_onkeydown(event) {
    ...
    if (event.keyCode != 13) {
      ...
      setTimeout(function () {
        ...
        console.log(vi_tri_con_tro_khi_di_chuyen_trong_double_click_input); // nếu vị trí click khi có công thức thêm vào array_2d[][] không nằm trong khung nhìn thì không làm gì ngược lại viết vào khung nhìn
        ...
        if (i_array_2d <= vi_tri_click_in_Data[0] - limit_view || i_array_2d >= vi_tri_click_in_Data[0] || j_array_2d <= vi_tri_click_in_Data[1] - limit_col_view || j_array_2d >= vi_tri_click_in_Data[1]) {} else {
          ...
          if (input_ === undefined) {
            ...
            var _input_ = parent_input.children[0]; // biến này dùng để xác định ảnh hưởng tới scoll

            xuat_hien_the_input = true;

            if (length_ >= 100) {
              _input_.style.width = length_ + 'px';
            }

            ; // trước gán sự kiện keydown cho input thì ta phải tắt lắng nghe sự kiện onkedown cho 1 element table cha của input 

            onKeyDown_1_element = true;
            _input_.value = thanh_dia_chi_0.current.value;
            run_function_when_input_focus(_input_, vi_tri_click_in_Data[2], vi_tri_click_in_Data[3], i_array_2d, j_array_2d);
            setTimeout(function () {
              thanh_dia_chi_0.current.focus({
                preventScroll: true
              });
    ...
    if (event.keyCode === 13) {
      ...
      mien_select_array_2d[1] = vi_tri_click_in_Data[1]; // khi enter nếu vịtrí ô viết công thức không nằm trong khung nhìn thì di chuyển đến đó sau đó tính toán      
      ...
      if (i_array_2d <= vi_tri_click_in_Data[0] - limit_view || i_array_2d >= vi_tri_click_in_Data[0] || j_array_2d <= vi_tri_click_in_Data[1] - limit_col_view || j_array_2d >= vi_tri_click_in_Data[1]) {
        ...
        thanh_dia_chi_0_on_keydown = false; // nếu ô trước đó đang viết công thức thì onclick_tinh_toan đã được chuyển thành true
        ...
        console.log('ô tô màu' + (vi_tri_click_in_Data[2] + 1) + "," + vi_tri_click_in_Data[3]); // set địa chỉ ô click  sau hành động trên
  ...
  function thanh_dia_chi_onMouseDown(event) {
    ...
    console.log(vi_tri_click_in_Data); // khi ấn chuột trái vào thanh địa chỉ nếu vị trí tô màu không nằm trong khung nhìn thì cuộn để vị trí tô màu nằm trong khung nhìn
  ...
  function _onMouseMove(event) {
    ...
    document.onmousemove = function (event_window) {
      ...
      mouse_X = event_window.clientX; // vẽ biểu tượng fill
      ...
      var y_r1c0 = a.current.children[mien_select_quy_ve[2] + 1 + 1].children[mien_select_quy_ve[1] + 1].getBoundingClientRect().y;
      var ty_le_canvas_width = table_excel_width / (x_r0c1 - x_r0c0 - 4);
      var ty_le_canvas_height = table_excel_height / (y_r1c0 - y_r0c0 - 4); // nếu trạng thái fill = true tức là đã vẽ rồi không cần vẽ lại nữa
      ...
      if (event_window.buttons == 1 && mouse_Y > table_excel.current.getBoundingClientRect().y + table_excel.current.clientHeight && mouse_X > table_excel.current.getBoundingClientRect().x + table_excel.current.clientWidth) {
        ...
        myInterval_0 = setTimeout(function doSomething() {
          ...
          table_excel_scrollTop += click_scroll_dichuyen;
          myInterval = setTimeout(doSomething, 10);
        }, 10); // setTimeout ở đây để vẽ lại chạy sau các hàm khi scroll
        ...
        setTimeout(function () {
          ...
          var elem_j = index - 1; // chỉ vẽ lại khi vị trí chuột tới ô khác tương ứng
        ...
        setTimeout(function () {
          ...
          var elem_i = index - 1;
          var elem_j = limit_col_view; // chỉ vẽ lại khi vị trí chuột tới ô khác tương ứng
        ...
        setTimeout(function () {
          ...
          var elem_i = index - 1; // chỉ vẽ lại khi vị trí chuột tới ô khác tương ứng
  ...
  function css() {
    ...
    return {
      ...
      table_excel: {
        ...
        height: "".concat(table_excel_height, "px"),
        width: "".concat(table_excel_width, "px"),
        overflowY: "auto",
        overflowX: "auto"
      },
      bar_reference_row: {
      ...
      bar_reference_col: (_bar_reference_col = {
        width: "auto",
        textAlign: "center",
        paddingLeft: "4px",
        paddingRight: "4px",
        backgroundColor: "#d8dcd6",
        borderBottomStyle: "none"
      }, _defineProperty(_bar_reference_col, "textAlign", "center"), _defineProperty(_bar_reference_col, "border", "1px ridge #ccc"), _defineProperty(_bar_reference_col, "height", "20px"), _defineProperty(_bar_reference_col, "display", "table-cell"), _bar_reference_col),
      row_excel: {
        display: "table-row"
      },
      col_excel: {
  ...
  function pseudo() {
    var _class = [['.black', ':hover', ' {  color: hotpink ; border: 2px solid red; }']];
    var mySheet = document.getElementById("style").sheet;
    var obj = {};
    var len = _class.length;

    for (var index = 0; index < len; index++) {
      var x = _class[index][0].slice(1);

      sheet_style('.' + myname + x + _class[index][1], _class[index][2], mySheet);
      obj[x] = myname + x;
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      height: "".concat(data_lenght, "px"),
      width: "".concat(data_col_lenght, "px")
    },
    ref: a
  }, /*#__PURE__*/React.createElement("div", {
    style: css.row_excel
  ...
  }), " ", Data_show_0[0].map(function (cell, j) {
    return /*#__PURE__*/React.createElement("div", {
      "data-bar": "bar",
      style: css.bar_reference_row
    }, j);
  }), "  "), Data_show_0.map(function (row, i) {
    return /*#__PURE__*/React.createElement("div", {
      style: css.row_excel
    }, /*#__PURE__*/React.createElement("div", {
      style: css.bar_reference_col,
...
function sheet_style(string_selector, string_css, mySheet) {
  ...
  var array_style = mySheet.cssRules;
  var css = string_selector + string_css;
  var tim_thay_string_selector = false;
  var len = array_style.length;
